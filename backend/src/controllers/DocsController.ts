...
class DocsController {
  static async docs(req: Request, res: Response): Promise<any> {
    const docsRoutes: DocsRoute[] = [
      ...
      {
        route: '/movies',
        description: 'Sends mass of data to the database (POST with file: search in collections [ ./collection-insomnia-requests-and-responses.json ] containing the name Get Prize Range)',
        method: 'POST',
        MultipartForm: {
      ...
      },
      {
        ...
        description: 'Gets the set containing the producer with the greatest interval between two consecutive awards, and which got two awards faster, following the format specification defined in the ./documents/Especificação Backend.pdf (GET) file',
        method: 'GET'
      }
    ];
